...
const EmailVerification = ({ resendVerificationEmail }) => {
  ...
  return (
    <Box sx={{ 
      ...
      <Box sx={{ textAlign: 'center', padding: '2rem', backgroundColor: '#3A3A3A', borderRadius: '10px' }}>
        <Typography variant="h5" sx={{ color: '#FF6B6B', fontWeight: 'bold', marginBottom: '1rem' }}>
          Verify Your Email
        </Typography>
        ...
        {resendMessage && <Typography sx={{ color: '#FF6B6B', marginTop: '1rem' }}>{resendMessage}</Typography>}
      </Box>
    </Box>
  );
};
